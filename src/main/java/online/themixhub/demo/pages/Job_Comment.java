...
public class Job_Comment {
	...
	public Result getPagePost(Request req) throws IOException {
		...
		if (!req.session().isSet("set")) {
			...
			LoggingUtil.insertUserLog(ds, "Jobs", "User attemped to post job comment: " + jobCommentForm.comments, account.getId(), req);
			...
			newComment.setJob_id(Integer.parseInt(jobCommentForm.jobID));
			if (jobCommentForm.comments != null)
				newComment.setComment(jobCommentForm.comments);
			if (moveTo != null && moveTo.exists())
				newComment.setFilepathsCSV(moveTo.getAbsolutePath());
