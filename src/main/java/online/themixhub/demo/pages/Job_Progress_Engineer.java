	...
	public Result getPage(Request req) throws IOException {
		...
		if (!req.session().isSet("set")) {
			...
			if(PermissionUtils.isEngineer(account)) {
				if(job.getStage() == Stage.ENGINEER_TIER_3_WAITING)
					MySQL.getJobs(ds).setJobStage(id, job.getEngineer_id(), Stage.ENGINEER_JOB_FINISHED.getID());
				else
					MySQL.getJobs(ds).setJobStage(id, job.getEngineer_id(), (job.getStage().getID() + 1));
